...
def load_lepe(trange=['2017-03-27', '2017-03-28'],
         pathformat=None,
         instrument='mgf',
         datatype='8sec',
         mode=None,
    ...
    # version control (valid major version > v05)
    file_array = np.array(out_files)
    major_version_info = np.zeros([len(out_files)],dtype='int64')
    control_version_number = 5
    if (len(out_files)) > 0:
        for test_n in range(0,len(out_files)):
            major_verion = int(Path(out_files[test_n]).stem.split('v')[1].split('_')[0])
            major_version_info[test_n] = major_verion
        
        index = np.where(major_version_info >= control_version_number)
        if (len(index[0]) != len(out_files)): print("The CDF files with the invalid major version is discarded in the loading file list.")
        out_files=file_array[index].tolist()
