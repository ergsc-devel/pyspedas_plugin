...
# Mar. 17, 2023 by T.H.
# This function is temporarily forked from https://github.com/MAVENSDC/PyTplot/blob/matplotlib-backend/pytplot/tplot_math/degap.py .
# The purpose is to avoid errors in numpy methods due to incompatible type of datetime64 and float.
# Currently, this forked (slightly modified) function is used by erg_interpolate_att, until the bug in the original
# degap.py is fixed.
...
def degap(tvar,dt,margin,func='nan',new_tvar = None):
    ...
    #gap_size = np.diff(pytplot.data_quants[tvar].coords['time']) #Commented out as it causes a type incompatible error

    new_tvar_index = pytplot.get_data( tvar )[0]
    gap_size = np.diff( new_tvar_index )
    gap_index_locations = np.where(gap_size > dt+margin)
    #new_tvar_index = pytplot.data_quants[tvar].coords['time'] #Commented out as this should be obtained by get_data()
