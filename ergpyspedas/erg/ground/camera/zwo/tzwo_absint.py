...
def tzwo_absint(
    ...
    v_name1 = f'zwo_asi_{site}_{wavelength[0]}_image_raw'
    ...
    times_bg, bg_data = get_data(f'zwo_asi_{site}_{wavelength[1]}_image_raw')

    # airglow exposure time data
    _, exp_ag_data = get_data(f'zwo_asi_{site}_{wavelength[0]}_exposure_time')

    # background exposure time data
    _, exp_bg_data = get_data(f'zwo_asi_{site}_{wavelength[1]}_exposure_time')
    ...
    # UNIX time in UT
    date = times_ag[0]
    # obtain the search result of ZWO calibration file
    calibration = search_zwo_calibration_file(date, site, wavelength[0], wid_cdf)
    ...
    if site == 'zug':
        ...
        cal_ag0 = np.roll(cal_ag0, -5, axis=1)
    ...
    if site == 'zug':
        cal_bg0 = np.roll(cal_bg0, 60, axis=0)
        cal_bg0 = np.roll(cal_bg0, -5, axis=1)
    ...
    for i in range(len(times_ag)):
        ...
        # star remover
        img_ag0 = ag_data[i]
        img_ag = rm_star_zwo_absint(img_ag0, wid_cdf)
 ...
 #       img_bg_int = np.zeros((wid_cdf, wid_cdf), dtype=float)
        ...
        # star remover
        img_bgs = rm_star_zwo_absint(img_bg0, wid_cdf)
